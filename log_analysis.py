...
def compare_invalid_IPs():
    ...
    for ip in invalid_ips:
        if ip in scanning_ips:
            both.append(ip)
    print(f"Unique scanning IPs: {len(set(scanning_ips))}")
    print(f"Unique overlap IPs (invalid login + scan): {len(set(both))}")
    print("Top 10 overlap IPs:", list(sorted(set(both)))[:10])
